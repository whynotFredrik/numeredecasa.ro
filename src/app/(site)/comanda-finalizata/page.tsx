...
import { CheckCircle2, Clock, XCircle, AlertCircle, Truck, PackageCheck } from 'lucide-react';
import Link from 'next/link';
import { Suspense, useEffect, useState } from 'react';

type OrderStatus = 'loading' | 'paid' | 'pending' | 'failed' | 'ramburs' | 'not_found';
  ...
  useEffect(() => {
    ...
    async function checkStatus() {
      try {
        ...
        // Plata ramburs - nu așteptăm confirmarea plății, comanda e validă imediat
        if (data.paymentMethod === 'ramburs') {
          setStatus('ramburs');
          return;
        }
      ...
      <main className="min-h-screen pt-32 pb-12 px-6 flex items-center justify-center bg-foreground/[0.02]">
        <div className="text-center space-y-6 max-w-lg">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center pt-4">
            <Link
              href={`/urmarire-comanda?orderId=${orderId}`}
              className="bg-primary text-white font-bold px-8 py-4 rounded-xl hover:bg-primary/90 transition-colors flex items-center justify-center gap-2"
            >
    ...
    return (
      <main className="min-h-screen pt-32 pb-12 px-6 flex items-center justify-center bg-foreground/[0.02]">
        <div className="text-center space-y-6 max-w-lg">
          ...
          <p className="text-foreground/60">
            Îți mulțumim pentru comandă{firstName ? `, ${firstName}` : ''}. Comanda ta a fost înregistrată și este în curs de pregătire. Plata se va face la livrare, în numerar, către curier.
          </p>
          {orderId && (
            <div className="inline-block bg-foreground/5 rounded-xl px-6 py-3 text-sm">
              <span className="text-foreground/50">ID Comandă: </span>
